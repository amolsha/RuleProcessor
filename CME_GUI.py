...
def extractConceptualModel():
    ...
    # ***************************************
    postProcess(binaryRelationships)
    # ***************************************
...
# ***************************************
def postProcess(binaryRelationships):
    print(binaryRelationships)
    # for binaryRelationship in binaryRelationships:
...
def generateFacts(pKbFile):
    ...
    for token in doc:
        if token.pos != PUNCT:
            ...
            facts.add(token.pos_.lower() + "(" + tokenLemma + ").")
            facts.add(token.dep_.lower() + "(" + token.head.lemma_.lower() + "," + tokenLemma + ").")
...
app = QApplication([])
window = QMainWindow()
window.setWindowTitle("Conceptual Model Extraction")
...
vBoxLayoutUserStory.addWidget(txtUserStory)
